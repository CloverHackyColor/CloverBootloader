...
Modified by dmazar to be used as a driver in Clover's \EFI\drivers dir
to satisfy boot.efi on UEFIs that do not have SMBIOS protocol:
 - disabled calls to SmbiosTableConstruction()
 - allocates EfiBootServicesData instead of EfiReservedMemoryType
   since we need it only for boot.efi. Clover will add final table
   to Config. tables for OSX.
  ...
  )
{
  ...
  if (Record == NULL) {
	//
	// dmazar: will use Record == NULL as a trigger to install SMBIOS to conf. table
	// note: this call will not be used when installed as a driver inside Clover's drivers
	// dir since Clover wil install this table on it's own
	//
  ...
  )
{
  ...
  //
  // Do not install if protocol is already installed.
  //
  Status = gBS->LocateProtocol(&gEfiSmbiosProtocolGuid, NULL, (VOID**)&Smbios);
  if (Status == EFI_SUCCESS) {
    Print(L"EfiSmbiosProtocol already present.");
    return EFI_ALREADY_STARTED;
  }
