...
#define CPU_MODEL_SANDY_BRIDGE_EX	0x2D
...
#define cpu_type_t UINT8
#define cpu_subtype_t UINT8
...
#define bcopy(x,y,s) CopyMem(x,y,s)
...
static void cpuid_update_generic_info()
{
    ...
    CopyMem(&info_p->cpuid_vendor[0], (char *)&cpuid_reg[ebx],  4);
    CopyMem(&info_p->cpuid_vendor[8], (char *)&cpuid_reg[ecx],  4);
    CopyMem(&info_p->cpuid_vendor[4], (char *)&cpuid_reg[edx],  4);
    ...
    if (max_extid >= 0x80000004) {
        ...
        CopyMem(&str[0],  (char *)cpuid_reg, 16);
        do_cpuid(0x80000003, cpuid_reg);
        CopyMem(&str[16], (char *)cpuid_reg, 16);
        do_cpuid(0x80000004, cpuid_reg);
        CopyMem(&str[32], (char *)cpuid_reg, 16);
        for (p = str; *p != '\0'; p++) {
            if (*p != ' ') break;
        }
		/*
		 @return Destination
		*/ 
		AsciiStrnCpy(info_p->cpuid_brand_string, p, sizeof(info_p->cpuid_brand_string));
		
		ib = sizeof(info_p->cpuid_brand_string);
		for (p1 = info_p->cpuid_brand_string; *p != '\0'; p1++) {
			*p1 = *p++;
			if (!(--ib)) {
				break;
			}
		...
		*p1 = '\0';
        ...
        if (!AsciiStrnCmp(info_p->cpuid_brand_string, CPUID_STRING_UNKNOWN,
				min(sizeof(info_p->cpuid_brand_string), AsciiStrLen(CPUID_STRING_UNKNOWN) + 1))) {
						 /*
    ...
    info_p->cpuid_extmodel  = (UINT8)bitfield(cpuid_reg[eax], 19, 16);
    info_p->cpuid_extfamily = (UINT8)bitfield(cpuid_reg[eax], 27, 20);
    ...
    if (info_p->cpuid_extfeatures & CPUID_FEATURE_MONITOR) {
        ...
        info_p->cpuid_arch_perf_width = (UINT8)bitfield(cpuid_reg[eax],23,16);
        ...
        info_p->cpuid_arch_perf_fixed_width = (UINT8)bitfield(cpuid_reg[edx],12, 5);
	...
	if (info_p->core_count == 0) {  //still zero???
		info_p->core_count = 1;
