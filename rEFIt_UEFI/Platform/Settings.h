...
public:
  ...
  const HDA_OUTPUTS &
  operator=(const HDA_OUTPUTS &) = delete; // Can be defined if needed
  ~HDA_OUTPUTS() {}
};
...
public:
  ...
  XString8Array getSplittedName() const {
    ...
    for (size_t idx = 0; idx < splittedName.size(); ++idx) {
      ...
      while (name.length() > 4)
        name.deleteCharsAtPos(name.length() - 1);
      while (name.length() < 4)
        name.strcat('_');
    }
    for (size_t idx = 1; idx < splittedName.size(); ++idx) {
      splittedName.insertReferenceAtPos(
          splittedName.ExtractFromPos(idx), 0,
          true); // A swap method in XObjARray would be slightly better to avoid
                 // memcpy in XObjArray when an object is removed.
    }
    return splittedName;
  }
};
...
public:
  ...
  XBool isEqual(const ACPI_RENAME_DEVICE &other) const {
    if (!acpiName.isEqual(other.acpiName))
      return false;
  ...
  XString8 getRenameTo() const {
    if (renameTo.length() == 4)
      return renameTo;
    XString8 newName = renameTo;
    while (newName.length() > 4)
      newName.deleteCharsAtPos(newName.length() - 1);
    while (newName.length() < 4)
      newName.strcat('_');
    return newName;
  }
...
public: // temporary, must be protected:
  // member defined with _ prefix should not be accessed from outside. I left
  // them public for now for CompareCustomEntries()
  ...
  XBool isEqual(const CUSTOM_LOADER_SUBENTRY_SETTINGS &other) const {
    ...
    if (!(_Arguments == other._Arguments))
      return false;
    if (!(_AddArguments == other._AddArguments))
      return false;
    if (!(_FullTitle == other._FullTitle))
      return false;
    if (!(_Title == other._Title))
      return false;
    if (!(_NoCaches == other._NoCaches))
      return false;
    return true;
  }
  void takeValueFrom(const ConfigPlistClass::GUI_Class::GUI_Custom_Class::
                         GUI_Custom_SubEntry_Class &other) {
...
public:
  ...
  const CUSTOM_LOADER_SUBENTRY_SETTINGS &settings =
      CUSTOM_LOADER_SUBENTRY_SETTINGS();

  CUSTOM_LOADER_SUBENTRY(const CUSTOM_LOADER_ENTRY &_customLoaderEntry,
                         const CUSTOM_LOADER_SUBENTRY_SETTINGS &_settings)
      : parent(_customLoaderEntry), settings(_settings) {}
...
/*
 * Wrapper class to bring some syntaxic sugar : initialisation at construction,
...
public:
  ...
  EFI_GRAPHICS_OUTPUT_BLT_PIXELClass(
      const EFI_GRAPHICS_OUTPUT_BLT_PIXEL &other) {
    Blue = other.Blue;
    Green = other.Green;
    Red = other.Red;
...
public:
  ...
  EFI_GRAPHICS_OUTPUT_BLT_PIXELClass BootBgColor =
      EFI_GRAPHICS_OUTPUT_BLT_PIXELClass();
  INT8 InjectKexts = -1;
  undefinable_bool NoCaches = undefinable_bool();
  XObjArrayWithTakeValueFromXmlArray<
      CUSTOM_LOADER_SUBENTRY_SETTINGS,
      ConfigPlistClass::GUI_Class::GUI_Custom_Class::GUI_Custom_SubEntry_Class>
      SubEntriesSettings = XObjArrayWithTakeValueFromXmlArray<
          CUSTOM_LOADER_SUBENTRY_SETTINGS,
          ConfigPlistClass::GUI_Class::GUI_Custom_Class::
...
public:
  ...
  XBool isEqual(const CUSTOM_LOADER_ENTRY_SETTINGS &other) const {
    ...
    if (!(ImageData == other.ImageData))
      return false;
    if (!(DriveImageData == other.DriveImageData))
      return false;
    ...
    if (!(Arguments == other.Arguments))
      return false;
    if (!(AddArguments == other.AddArguments))
      return false;
    ...
    if (!(Hotkey == other.Hotkey))
      return false;
    if (!(CommonSettings == other.CommonSettings))
      return false;
    if (!(Hidden == other.Hidden))
      return false;
    if (!(AlwaysHidden == other.AlwaysHidden))
      return false;
    if (!(Type == other.Type))
      return false;
    if (!(VolumeType == other.VolumeType))
      return false;
    if (!(KernelScan == other.KernelScan))
      return false;
    if (!(CustomLogoAsXString8 == other.CustomLogoAsXString8))
      return false;
    if (!(CustomLogoAsData == other.CustomLogoAsData))
      return false;
    if (memcmp(&BootBgColor, &other.BootBgColor, sizeof(BootBgColor)) != 0)
      return false;
    if (!(InjectKexts == other.InjectKexts))
      return false;
    if (!(NoCaches == other.NoCaches))
      return false;
    if (!SubEntriesSettings.isEqual(other.SubEntriesSettings))
      return false;
    if (!(m_DriveImagePath == other.m_DriveImagePath))
      return false;
    if (!(m_Title == other.m_Title))
      return false;
    if (!(CustomLogoTypeSettings == other.CustomLogoTypeSettings))
      return false;
    if (!(m_ImagePath == other.m_ImagePath))
      return false;
    if (!(ForceTextMode == other.ForceTextMode))
      return false;
    return true;
  }
  void takeValueFrom(const ConfigPlistClass::GUI_Class::GUI_Custom_Class::
                         GUI_Custom_Entry_Class &other) {
    Disabled = other.dgetDisabled();
  ...
  const XStringW &dgetImagePath() const {
    if (m_ImagePath.notEmpty())
      return m_ImagePath;
    if (ImageData.notEmpty())
      return NullXStringW;
    if (OSTYPE_IS_OSX_RECOVERY(Type))
      return defaultRecoveryImagePath;
    return NullXStringW;
  }
  const XStringW &dgetDriveImagePath() const {
    if (m_DriveImagePath.notEmpty())
      return m_DriveImagePath;
    if (DriveImageData.notEmpty())
      return NullXStringW;
    if (OSTYPE_IS_OSX_RECOVERY(Type))
      return defaultRecoveryDriveImagePath;
    return NullXStringW;
  }
};
...
public:
  ...
  XObjArray<CUSTOM_LOADER_SUBENTRY> SubEntries =
      XObjArray<CUSTOM_LOADER_SUBENTRY>();
  XIcon Image = XIcon();
  ...
  UINT8 getFlags(XBool NoCachesDefault) const {
    UINT8 Flags = 0;
    if (settings.Arguments.isDefined())
      Flags = OSFLAG_SET(Flags, OSFLAG_NODEFAULTARGS);
    if (settings.AlwaysHidden)
      Flags = OSFLAG_SET(Flags, OSFLAG_DISABLED);
    if (settings.Type == OSTYPE_LIN)
      Flags = OSFLAG_SET(Flags, OSFLAG_NODEFAULTARGS);
    if (OSTYPE_IS_OSX(settings.Type) || OSTYPE_IS_OSX_RECOVERY(settings.Type) ||
        OSTYPE_IS_OSX_INSTALLER(settings.Type)) {
      Flags = OSFLAG_UNSET(Flags, OSFLAG_NOCACHES);
    }
    if (settings.NoCaches.isDefined()) {
      if (settings.NoCaches)
    ...
    if (SubEntries.notEmpty())
      Flags = OSFLAG_SET(Flags, OSFLAG_NODEFAULTMENU);
    return Flags;
  }
};
...
public:
  ...
  XBool isEqual(const CUSTOM_LEGACY_ENTRY_SETTINGS &other) const {
    ...
    if (!(ImageData == other.ImageData))
      return false;
    if (!(DriveImagePath == other.DriveImagePath))
      return false;
    if (!(DriveImageData == other.DriveImageData))
      return false;
    if (!(Volume == other.Volume))
...
public:
  ...
  CUSTOM_LEGACY_ENTRY(const CUSTOM_LEGACY_ENTRY_SETTINGS &_settings,
                      const EFI_FILE &ThemeDir)
      : settings(_settings) {
    ...
    if (settings.DriveImagePath.notEmpty()) {
      ...
      DriveImage.Image.FromPNG(settings.DriveImageData.data(),
                               settings.DriveImageData.size());
    }
  }

  UINT8 getFlags() const {
    UINT8 Flags = 0;
    if (settings.Disabled || settings.AlwaysHidden)
      Flags = OSFLAG_SET(Flags, OSFLAG_DISABLED);
    return Flags;
  }
};
...
public:
  ...
  void takeValueFrom(
      const ConfigPlistClass::GUI_Class::GUI_Custom_Class::GUI_Custom_Tool_Class
          &other) {
    Disabled = other.dgetDisabled();
    ImagePath = other.dgetImagePath();
    ImageData = other.dgetImageData();
...
public:
  ...
  CUSTOM_TOOL_ENTRY(const CUSTOM_TOOL_ENTRY_SETTINGS &_settings,
                    const EFI_FILE &ThemeDir)
      : settings(_settings) {
    if (settings.ImagePath.notEmpty()) {
      Image.LoadXImage(&ThemeDir, settings.ImagePath);
...
/*
 * From config.plist,
 * /Devices/Properties will construct a XObjArray<DEV_PROPERTY>, stored in
 * ArbProperties /Devices/Arbitrary/CustomProperties will construct a
 * XObjArray<DEV_PROPERTY>, stored in ArbProperties
 */
// 2021-04 Jief : this is the old structure. Kept here for be able to compare
// old and new code.
class DEV_PROPERTY {
public:
  UINT32 Device = 0;
  ...
  // Not sure if default are valid. Delete them. If needed, proper ones can be
  // created
...
/**
  ...
  this is supposed to be a replacement of VBIOS_PATCH_BYTES, but that would need
VbiosPatchLibrary to be update to C++. Quite easy, but need cpp_fundation to
become a library. TODO
**/
class VBIOS_PATCH {
public:
  ...
  void takeValueFrom(
      const ConfigPlistClass::Graphics_Class::Graphics_PatchVBiosBytes_Class
          &other) {
    Find = other.dgetFind();
...
class PatchVBiosBytesNewClass
    : public XObjArrayWithTakeValueFromXmlArray<
          VBIOS_PATCH,
          ConfigPlistClass::Graphics_Class::Graphics_PatchVBiosBytes_Class> {
  mutable XArray<VBIOS_PATCH_BYTES> VBIOS_PATCH_BYTES_array =
      XArray<VBIOS_PATCH_BYTES>();

public:
#if __cplusplus > 201703L
  XBool operator==(const PatchVBiosBytesNewClass &other) const {
    return XObjArray<VBIOS_PATCH>::operator==(other);
  }
#endif
  ...
  const VBIOS_PATCH_BYTES *getVBIOS_PATCH_BYTES() const {
    ...
    for (size_t idx = 0; idx < size(); ++idx) {
      ...
      VBIOS_PATCH_BYTES_array[idx].NumberOfBytes =
          ElementAt(idx).Replace.size();
    }
    return VBIOS_PATCH_BYTES_array;
  }
  size_t getVBIOS_PATCH_BYTES_count() const { return size(); }
...
public:
  ...
  public:
    ...
    XBool isEqual(const BootClass &other) const {
      if (!(Timeout == other.Timeout))
        return false;
      if (!(SkipHibernateTimeout == other.SkipHibernateTimeout))
        return false;
      if (!(DisableCloverHotkeys == other.DisableCloverHotkeys))
        return false;
      if (!(BootArgs == other.BootArgs))
        return false;
      if (!(NeverDoRecovery == other.NeverDoRecovery))
        return false;
      if (!(LastBootedVolume == other.LastBootedVolume))
        return false;
      if (!(DefaultVolume == other.DefaultVolume))
        return false;
      if (!(DefaultLoader == other.DefaultLoader))
        return false;
      if (!(DebugLog == other.DebugLog))
        return false;
      if (!(FastBoot == other.FastBoot))
        return false;
      if (!(NoEarlyProgress == other.NoEarlyProgress))
        return false;
      if (!(NeverHibernate == other.NeverHibernate))
        return false;
      if (!(StrictHibernate == other.StrictHibernate))
        return false;
      if (!(RtcHibernateAware == other.RtcHibernateAware))
        return false;
      if (!(HibernationFixup == other.HibernationFixup))
        return false;
      if (!(SignatureFixup == other.SignatureFixup))
        return false;
      if (!(SecureSetting == other.SecureSetting))
        return false;
      if (!(SecureBootPolicy == other.SecureBootPolicy))
        return false;
      if (!(SecureBootWhiteList == other.SecureBootWhiteList))
        return false;
      if (!(SecureBootBlackList == other.SecureBootBlackList))
        return false;
      if (!(XMPDetection == other.XMPDetection))
        return false;
      if (!(LegacyBoot == other.LegacyBoot))
        return false;
      if (!(LegacyBiosDefaultEntry == other.LegacyBiosDefaultEntry))
        return false;
      if (!(CustomLogoType == other.CustomLogoType))
        return false;
      if (!(CustomLogoAsXString8 == other.CustomLogoAsXString8))
        return false;
      if (!(CustomLogoAsData == other.CustomLogoAsData))
  ...
  public:
    ...
    public:
      ...
      XBool isEqual(const ACPIDropTablesClass &other) const {
        ...
        if (!(TableId == other.TableId))
          return false;
        if (!(TabLength == other.TabLength))
          return false;
        if (!(OtherOS == other.OtherOS))
          return false;
        return true;
      }
      void takeValueFrom(
          const ConfigPlistClass::ACPI_Class::ACPI_DropTables_Class &other) {
    ...
    public:
      ...
      public:
        ...
        INPUT_ITEM PatchDsdtMenuItem =
            INPUT_ITEM(); // Not read from config.plist. Should be moved out.
        uint64_t Skip = 0;
        int Count = 0;
        ...
        XBool isEqual(const DSDT_Patch &other) const {
          if (!(Disabled == other.Disabled))
            return false;
          if (!(PatchDsdtLabel == other.PatchDsdtLabel))
            return false;
          if (!(PatchDsdtFind == other.PatchDsdtFind))
            return false;
          if (!(PatchDsdtReplace == other.PatchDsdtReplace))
            return false;
          if (!(PatchDsdtTgt == other.PatchDsdtTgt))
            return false;
          if (!(PatchDsdtMenuItem == other.PatchDsdtMenuItem))
            return false;
          if (!(Skip == other.Skip))
            return false;
        ...
        void takeValueFrom(const ConfigPlistClass::ACPI_Class::DSDT_Class::
                               ACPI_DSDT_Patch_Class &other) {
          Disabled = other.dgetDisabled();
      ...
      XObjArrayWithTakeValueFromXmlArray<
          DSDT_Patch,
          ConfigPlistClass::ACPI_Class::DSDT_Class::ACPI_DSDT_Patch_Class>
          DSDTPatchArray = XObjArrayWithTakeValueFromXmlArray<
              DSDT_Patch, ConfigPlistClass::ACPI_Class::DSDT_Class::
                              ACPI_DSDT_Patch_Class>();

#if __cplusplus > 201703L
      XBool operator==(const DSDTClass &) const = default;
#endif
      XBool isEqual(const DSDTClass &other) const {
        if (!(DsdtName == other.DsdtName))
          return false;
        if (!(DebugDSDT == other.DebugDSDT))
          return false;
        if (!(Rtc8Allowed == other.Rtc8Allowed))
          return false;
        if (!(PNLF_UID == other.PNLF_UID))
          return false;
        if (!(FixDsdt == other.FixDsdt))
          return false;
        if (!(ReuseFFFF == other.ReuseFFFF))
          return false;
        if (!(SuspendOverride == other.SuspendOverride))
          return false;
        if (!DSDTPatchArray.isEqual(other.DSDTPatchArray))
          return false;
        return true;
      }
      void
      takeValueFrom(const ConfigPlistClass::ACPI_Class::DSDT_Class &other) {
    ...
    public:
      ...
      public:
        ...
        XBool isEqual(const GenerateClass &other) const {
          if (!(GeneratePStates == other.GeneratePStates))
            return false;
          if (!(GenerateCStates == other.GenerateCStates))
            return false;
          if (!(GenerateAPSN == other.GenerateAPSN))
            return false;
          if (!(GenerateAPLF == other.GenerateAPLF))
            return false;
          if (!(GeneratePluginType == other.GeneratePluginType))
            return false;
          return true;
        }
        void takeValueFrom(
            const ConfigPlistClass::ACPI_Class::SSDT_Class::XmlUnionGenerate
                &other) {
      ...
      XBool isEqual(const SSDTClass &other) const {
        if (!(DropSSDTSetting == other.DropSSDTSetting))
          return false;
        if (!(NoOemTableId == other.NoOemTableId))
          return false;
        if (!(NoDynamicExtract == other.NoDynamicExtract))
          return false;
        if (!(EnableISS == other.EnableISS))
          return false;
        if (!(EnableC7 == other.EnableC7))
          return false;
        if (!(_EnableC6 == other._EnableC6))
          return false;
        if (!(_EnableC4 == other._EnableC4))
          return false;
        if (!(_EnableC2 == other._EnableC2))
          return false;
        if (!(_C3Latency == other._C3Latency))
          return false;
        if (!(PLimitDict == other.PLimitDict))
          return false;
        if (!(UnderVoltStep == other.UnderVoltStep))
          return false;
        if (!(DoubleFirstState == other.DoubleFirstState))
          return false;
        if (!(MinMultiplier == other.MinMultiplier))
          return false;
        if (!(MaxMultiplier == other.MaxMultiplier))
          return false;
        if (!(PluginType == other.PluginType))
          return false;
        if (!Generate.isEqual(other.Generate))
          return false;
        return true;
      }
      void
      takeValueFrom(const ConfigPlistClass::ACPI_Class::SSDT_Class &other) {
    ...
    //      XObjArray<ACPI_RENAME_DEVICE>     DeviceRename =
    //      XObjArray<ACPI_RENAME_DEVICE>();
    //      XObjArrayWithTakeValueFrom<ACPI_RENAME_DEVICE,
    //      ConfigPlistClass::ACPI_Class::ACPI_RenamesDevices_Class>
    //                                        DeviceRename =
    //                                        XObjArrayWithTakeValueFrom<ACPI_RENAME_DEVICE,
    //                                        ConfigPlistClass::ACPI_Class::ACPI_RenamesDevices_Class>();
    ...
    public:
      void takeValueFrom(
          const ConfigPlistClass::ACPI_Class::ACPI_RenamesDevices_Class
              &other) {
        size_t idx;
        for (idx = 0; idx < other.size(); ++idx) {
          if (idx < size())
            ElementAt(idx).takeValueFrom(other.getAtIndex(idx));
          else {
    ...
    XObjArrayWithTakeValueFromXmlArray<
        ACPIDropTablesClass,
        ConfigPlistClass::ACPI_Class::ACPI_DropTables_Class>
        ACPIDropTablesArray = XObjArrayWithTakeValueFromXmlArray<
            ACPIDropTablesClass,
            ConfigPlistClass::ACPI_Class::ACPI_DropTables_Class>();
    ...
    XBool isEqual(const ACPIClass &other) const {
      if (!(ResetAddr == other.ResetAddr))
        return false;
      if (!(ResetVal == other.ResetVal))
        return false;
      if (!(SlpSmiEnable == other.SlpSmiEnable))
        return false;
      if (!(FixHeaders == other.FixHeaders))
        return false;
      if (!(FixMCFG == other.FixMCFG))
        return false;
      if (!(NoASPM == other.NoASPM))
        return false;
      if (!(smartUPS == other.smartUPS))
        return false;
      if (!(PatchNMI == other.PatchNMI))
        return false;
      if (!(AutoMerge == other.AutoMerge))
        return false;
      if (!(DisabledAML == other.DisabledAML))
        return false;
      if (!(SortedACPI == other.SortedACPI))
        return false;
      if (!DeviceRename.isEqual(other.DeviceRename))
        return false;
      if (!ACPIDropTablesArray.isEqual(other.ACPIDropTablesArray))
        return false;
      if (!DSDT.isEqual(other.DSDT))
        return false;
      if (!SSDT.isEqual(other.SSDT))
        return false;
      return true;
    }
  ...
  public:
    ...
    public:
      ...
      XBool isEqual(const MouseClass &other) const {
        if (!(PointerSpeed == other.PointerSpeed))
          return false;
        if (!(PointerEnabled == other.PointerEnabled))
          return false;
        if (!(DoubleClickTime == other.DoubleClickTime))
          return false;
        if (!(PointerMirror == other.PointerMirror))
          return false;
        return true;
      }
      void
      takeValueFrom(const ConfigPlistClass::GUI_Class::GUI_Mouse_Class &other) {
    ...
    public:
      ...
      XBool isEqual(const ScanClass &other) const {
        if (!(DisableEntryScan == other.DisableEntryScan))
          return false;
        if (!(DisableToolScan == other.DisableToolScan))
          return false;
        if (!(KernelScan == other.KernelScan))
          return false;
        if (!(LinuxScan == other.LinuxScan))
          return false;
        if (!(LegacyFirst == other.LegacyFirst))
          return false;
        if (!(NoLegacy == other.NoLegacy))
          return false;
        return true;
      }
      void
      takeValueFrom(const ConfigPlistClass::GUI_Class::GUI_Scan_Class &other) {
    ...
    XObjArrayWithTakeValueFromXmlArray<
        CUSTOM_LOADER_ENTRY_SETTINGS,
        ConfigPlistClass::GUI_Class::GUI_Custom_Class::GUI_Custom_Entry_Class>
        CustomEntriesSettings = XObjArrayWithTakeValueFromXmlArray<
            CUSTOM_LOADER_ENTRY_SETTINGS,
            ConfigPlistClass::GUI_Class::GUI_Custom_Class::
                GUI_Custom_Entry_Class>();
    XObjArrayWithTakeValueFromXmlArray<
        CUSTOM_LEGACY_ENTRY_SETTINGS,
        ConfigPlistClass::GUI_Class::GUI_Custom_Class::GUI_Custom_Legacy_Class>
        CustomLegacySettings = XObjArrayWithTakeValueFromXmlArray<
            CUSTOM_LEGACY_ENTRY_SETTINGS,
            ConfigPlistClass::GUI_Class::GUI_Custom_Class::
                GUI_Custom_Legacy_Class>();
    XObjArrayWithTakeValueFromXmlArray<
        CUSTOM_TOOL_ENTRY_SETTINGS,
        ConfigPlistClass::GUI_Class::GUI_Custom_Class::GUI_Custom_Tool_Class>
        CustomToolSettings = XObjArrayWithTakeValueFromXmlArray<
            CUSTOM_TOOL_ENTRY_SETTINGS,
            ConfigPlistClass::GUI_Class::GUI_Custom_Class::
                GUI_Custom_Tool_Class>();
    ...
    XBool isEqual(const GUIClass &other) const {
      ...
      if (!(Theme == other.Theme))
        return false;
      if (!(EmbeddedThemeType == other.EmbeddedThemeType))
        return false;
      if (!(PlayAsync == other.PlayAsync))
        return false;
      if (!(CustomIcons == other.CustomIcons))
        return false;
      if (!(TextOnly == other.TextOnly))
        return false;
      if (!(ShowOptimus == other.ShowOptimus))
        return false;
      if (!(ScreenResolution == other.ScreenResolution))
        return false;
      if (!(ProvideConsoleGop == other.ProvideConsoleGop))
        return false;
      if (!(ConsoleMode == other.ConsoleMode))
        return false;
      if (!(Language == other.Language))
        return false;
      if (!(languageCode == other.languageCode))
        return false;
      if (!(KbdPrevLang == other.KbdPrevLang))
        return false;
      if (!(HVHideStrings == other.HVHideStrings))
        return false;
      if (!Scan.isEqual(other.Scan))
        return false;
      if (!Mouse.isEqual(other.Mouse))
        return false;
      if (!CustomEntriesSettings.isEqual(other.CustomEntriesSettings))
        return false;
      if (!CustomLegacySettings.isEqual(other.CustomLegacySettings))
        return false;
      if (!CustomToolSettings.isEqual(other.CustomToolSettings))
        return false;
      return true;
    }
  ...
  public:
    ...
    XBool isEqual(const CPUClass &other) const {
      if (!(QPI == other.QPI))
        return false;
      if (!(CpuFreqMHz == other.CpuFreqMHz))
        return false;
      if (!(CpuType == other.CpuType))
        return false;
      if (!(QEMU == other.QEMU))
        return false;
      if (!(UseARTFreq == other.UseARTFreq))
        return false;
      if (!(BusSpeed == other.BusSpeed))
        return false;
      if (!(UserChange == other.UserChange))
        return false;
      if (!(SavingMode == other.SavingMode))
        return false;
      if (!(HWPEnable == other.HWPEnable))
        return false;
      if (!(HWPValue == other.HWPValue))
        return false;
      if (!(TDP == other.TDP))
        return false;
      if (!(TurboDisabled == other.TurboDisabled))
        return false;
      if (!(_EnableC6 == other._EnableC6))
        return false;
      if (!(_EnableC4 == other._EnableC4))
  ...
  public:
    ...
    XBool isEqual(const SystemParametersClass &other) const {
      if (!(WithKexts == other.WithKexts))
        return false;
      if (!(WithKextsIfNoFakeSMC == other.WithKextsIfNoFakeSMC))
        return false;
      if (!(NoCaches == other.NoCaches))
        return false;
      if (!(BacklightLevel == other.BacklightLevel))
        return false;
      if (!(BacklightLevelConfig == other.BacklightLevelConfig))
        return false;
      if (!(CustomUuid == other.CustomUuid))
        return false;
      if (!(_InjectSystemID == other._InjectSystemID))
        return false;
      if (!(NvidiaWeb == other.NvidiaWeb))
        return false;
      return true;
    }
  ...
  public:
    ...
    public:
      ...
      XBool isEqual(const EDIDClass &other) const {
        if (!(InjectEDID == other.InjectEDID))
          return false;
        if (!(CustomEDID == other.CustomEDID))
          return false;
        if (!(VendorEDID == other.VendorEDID))
          return false;
        if (!(ProductEDID == other.ProductEDID))
          return false;
        if (!(EdidFixHorizontalSyncPulseWidth ==
              other.EdidFixHorizontalSyncPulseWidth))
          return false;
        if (!(EdidFixVideoInputSignal == other.EdidFixVideoInputSignal))
          return false;
        return true;
      }
      void takeValueFrom(
          const ConfigPlistClass::Graphics_Class::Graphics_EDID_Class &other) {
        ...
        EdidFixHorizontalSyncPulseWidth =
            other.dgetEdidFixHorizontalSyncPulseWidth();
    ...
    public:
      ...
      XBool isEqual(const InjectAsDictClass &other) const {
        if (!(GraphicsInjector == other.GraphicsInjector))
          return false;
        if (!(InjectIntel == other.InjectIntel))
          return false;
        if (!(InjectATI == other.InjectATI))
          return false;
        if (!(InjectNVidia == other.InjectNVidia))
          return false;
        return true;
      }
      void takeValueFrom(
          const ConfigPlistClass::Graphics_Class::XmlInjectUnion &other) {
    ...
    public:
      ...
      XBool isEqual(const GRAPHIC_CARD &other) const {
        ...
        if (!(SubId == other.SubId))
          return false;
        if (!(VideoRam == other.VideoRam))
          return false;
        if (!(VideoPorts == other.VideoPorts))
          return false;
      ...
      void takeValueFrom(
          const ConfigPlistClass::Graphics_Class::Graphics_ATI_NVIDIA_Class
              &other) {
        Signature = other.dgetSignature();
    ...
    XObjArrayWithTakeValueFromXmlArray<
        GRAPHIC_CARD,
        ConfigPlistClass::Graphics_Class::Graphics_ATI_NVIDIA_Class>
        ATICardList = XObjArrayWithTakeValueFromXmlArray<
            GRAPHIC_CARD,
            ConfigPlistClass::Graphics_Class::Graphics_ATI_NVIDIA_Class>();
    XObjArrayWithTakeValueFromXmlArray<
        GRAPHIC_CARD,
        ConfigPlistClass::Graphics_Class::Graphics_ATI_NVIDIA_Class>
        NVIDIACardList = XObjArrayWithTakeValueFromXmlArray<
            GRAPHIC_CARD,
            ConfigPlistClass::Graphics_Class::Graphics_ATI_NVIDIA_Class>();
    ...
    XBool isEqual(const GraphicsClass &other) const {
      if (!(PatchVBios == other.PatchVBios))
        return false;
      if (!PatchVBiosBytes.isEqual(other.PatchVBiosBytes))
        return false;
      if (!(RadeonDeInit == other.RadeonDeInit))
        return false;
      if (!(LoadVBios == other.LoadVBios))
        return false;
      if (!(VRAM == other.VRAM))
        return false;
      if (!(RefCLK == other.RefCLK))
        return false;
      if (!(FBName == other.FBName))
        return false;
      if (!(VideoPorts == other.VideoPorts))
        return false;
      if (!(NvidiaGeneric == other.NvidiaGeneric))
        return false;
      if (!(NvidiaNoEFI == other.NvidiaNoEFI))
        return false;
      if (!(NvidiaSingle == other.NvidiaSingle))
        return false;
      if (!(Dcfg == other.Dcfg))
        return false;
      if (!(NVCAP == other.NVCAP))
        return false;
      if (!(BootDisplay == other.BootDisplay))
        return false;
      if (!(DualLink == other.DualLink))
        return false;
      if (!(_IgPlatform == other._IgPlatform))
        return false;
      if (!EDID.isEqual(other.EDID))
        return false;
      if (!InjectAsDict.isEqual(other.InjectAsDict))
        return false;
      if (!ATICardList.isEqual(other.ATICardList))
        return false;
      if (!NVIDIACardList.isEqual(other.NVIDIACardList))
        return false;
      return true;
    }
    ...
    // XBool getGraphicsInjector() const { return InjectAsBool.isDefined() ?
    // InjectAsBool.value() : InjectAsDict.GraphicsInjector; } XBool
    // InjectIntel() const { return InjectAsBool.isDefined() ?
    // InjectAsBool.value() : InjectAsDict.InjectIntel; } XBool InjectATI()
    // const { return InjectAsBool.isDefined() ? InjectAsBool.value() :
    // InjectAsDict.InjectATI; } XBool InjectNVidia() const { return
    // InjectAsBool.isDefined() ? InjectAsBool.value() :
    // InjectAsDict.InjectNVidia; }
  };
  ...
  public:
    ...
    public:
      ...
      XBool isEqual(const AudioClass &other) const {
        if (!(ResetHDA == other.ResetHDA))
          return false;
        if (!(HDAInjection == other.HDAInjection))
          return false;
        if (!(HDALayoutId == other.HDALayoutId))
          return false;
        if (!(AFGLowPowerState == other.AFGLowPowerState))
          return false;
        return true;
      }
      void takeValueFrom(
          const ConfigPlistClass::DevicesClass::Devices_Audio_Class &other) {
    ...
    public:
      ...
      XBool isEqual(const USBClass &other) const {
        if (!(USBInjection == other.USBInjection))
          return false;
        if (!(USBFixOwnership == other.USBFixOwnership))
          return false;
        if (!(InjectClockID == other.InjectClockID))
          return false;
        if (!(HighCurrent == other.HighCurrent))
          return false;
        if (!(NameEH00 == other.NameEH00))
          return false;
        if (!(NameXH00 == other.NameXH00))
          return false;
        return true;
      }
      void takeValueFrom(
          const ConfigPlistClass::DevicesClass::Devices_USB_Class &other) {
    ...
    public:
      ...
      XBool isEqual(const AddPropertyClass &other) const {
        ...
        if (!(MenuItem == other.MenuItem))
          return false;
        //          if ( !(DevicePathAsString == other.DevicePathAsString) )
        //          return false; if ( !(Label == other.Label) ) return false;
        return true;
      }
      void takeValueFrom(
          const ConfigPlistClass::DevicesClass::Devices_AddProperties_Dict_Class
              &other) {
    ...
    public:
      ...
      void takeValueFrom(
          const ConfigPlistClass::DevicesClass::SimplePropertyClass_Class
              &other) {
        Key = other.dgetKey();
        Value = other.dgetValue();
        ValueType = other.dgetValueType();
      ...
      void takeValueFrom(
          const ConfigPlistClass::DevicesClass::PropertiesUnion::Property
              &other) {
        Key = other.dgetKey();
        Value = other.dgetValue();
        ValueType = other.dgetValueType();
    ...
    // Property don't have Device. Before it was always Device = 0 to
    // differentiate from Arbitrary properties.
    ...
    public:
      ...
      public:
        ...
        // XString8                     Label = XString8(); // Label is the same
        // as DevicePathAsString, so it's not needed.
        XObjArrayWithTakeValueFromXmlRepeatingDict<
            SimplePropertyClass,
            ConfigPlistClass::DevicesClass::PropertiesUnion::Property>
            propertiesArray = XObjArrayWithTakeValueFromXmlRepeatingDict<
                SimplePropertyClass,
                ConfigPlistClass::DevicesClass::PropertiesUnion::Property>();
        ...
        EFI_DEVICE_PATH_PROTOCOL *getDevicePath() const {
          ...
          if (DevicePathAsString.isEqualIC("PrimaryGPU") &&
              gConf.GfxPropertiesArray.size() > 0) {
            DevicePath = DevicePathFromHandle(
                gConf.GfxPropertiesArray[0].Handle); // first gpu
          } else if (DevicePathAsString.isEqualIC("SecondaryGPU") &&
                     gConf.GfxPropertiesArray.size() > 1) {
            DevicePath = DevicePathFromHandle(
                gConf.GfxPropertiesArray[1].Handle); // second gpu
          } else {
            DevicePath =
                ConvertTextToDevicePath(DevicePathAsString.wc_str()); // TODO
          }
          return DevicePath;
        }
#endif
        ...
        XBool isEqual(const PropertyClass &other) const {
          ...
          if (!propertiesArray.isEqual(other.propertiesArray))
            return false;
          return true;
        }
        void takeValueFrom(const ConfigPlistClass::DevicesClass::
                               PropertiesUnion::Properties4DeviceClass &other) {
      ...
      XObjArrayWithTakeValueFromXmlRepeatingDict<
          PropertyClass, ConfigPlistClass::DevicesClass::PropertiesUnion::
                             Properties4DeviceClass>
      ...
      XBool isEqual(const PropertiesClass &other) const {
        if (!(propertiesAsString == other.propertiesAsString))
          return false;
        if (!PropertyArray.isEqual(other.PropertyArray))
          return false;
        return true;
      }
      void takeValueFrom(
          const ConfigPlistClass::DevicesClass::PropertiesUnion &other) {
    ...
    public:
      ...
      XObjArrayWithTakeValueFromXmlArray<
          SimplePropertyClass,
          ConfigPlistClass::DevicesClass::SimplePropertyClass_Class>
          CustomPropertyArray = XObjArrayWithTakeValueFromXmlArray<
              SimplePropertyClass,
              ConfigPlistClass::DevicesClass::SimplePropertyClass_Class>();
      ...
      XBool isEqual(const ArbitraryPropertyClass &other) const {
        ...
        if (!(Label == other.Label))
          return false;
        if (!CustomPropertyArray.isEqual(other.CustomPropertyArray))
          return false;
        return true;
      }
      void takeValueFrom(
          const ConfigPlistClass::DevicesClass::Devices_Arbitrary_Class
              &other) {
        Device = other.dgetDevice();
        Label = other.dgetLabel();
    ...
    public:
      ...
      XBool isEqual(const FakeIDClass &other) const {
        if (!(FakeATI == other.FakeATI))
          return false;
        if (!(FakeNVidia == other.FakeNVidia))
          return false;
        if (!(FakeIntel == other.FakeIntel))
          return false;
        if (!(FakeLAN == other.FakeLAN))
          return false;
        if (!(FakeWIFI == other.FakeWIFI))
          return false;
        if (!(FakeSATA == other.FakeSATA))
          return false;
        if (!(FakeXHCI == other.FakeXHCI))
          return false;
        if (!(FakeIMEI == other.FakeIMEI))
          return false;
        return true;
      }
      void takeValueFrom(
          const ConfigPlistClass::DevicesClass::Devices_FakeID_Class &other) {
    ...
    XObjArrayWithTakeValueFromXmlArray<
        AddPropertyClass,
        ConfigPlistClass::DevicesClass::Devices_AddProperties_Dict_Class>
        AddPropertyArray = XObjArrayWithTakeValueFromXmlArray<
            AddPropertyClass,
            ConfigPlistClass::DevicesClass::Devices_AddProperties_Dict_Class>();
    PropertiesClass Properties = PropertiesClass();
    XObjArrayWithTakeValueFromXmlArray<
        ArbitraryPropertyClass,
        ConfigPlistClass::DevicesClass::Devices_Arbitrary_Class>
        ArbitraryArray = XObjArrayWithTakeValueFromXmlArray<
            ArbitraryPropertyClass,
            ConfigPlistClass::DevicesClass::Devices_Arbitrary_Class>();
    ...
    XBool isEqual(const DevicesClass &other) const {
      if (!(StringInjector == other.StringInjector))
        return false;
      if (!(IntelMaxBacklight == other.IntelMaxBacklight))
        return false;
      if (!(IntelBacklight == other.IntelBacklight))
        return false;
      if (!(IntelMaxValue == other.IntelMaxValue))
        return false;
      if (!(LANInjection == other.LANInjection))
        return false;
      if (!(HDMIInjection == other.HDMIInjection))
        return false;
      if (!(NoDefaultProperties == other.NoDefaultProperties))
        return false;
      if (!(UseIntelHDMI == other.UseIntelHDMI))
        return false;
      if (!(ForceHPET == other.ForceHPET))
        return false;
      if (!(DisableFunctions == other.DisableFunctions))
        return false;
      if (!(AirportBridgeDeviceName == other.AirportBridgeDeviceName))
        return false;
      if (!Audio.isEqual(other.Audio))
        return false;
      if (!USB.isEqual(other.USB))
        return false;
      if (!FakeID.isEqual(other.FakeID))
        return false;
      if (!AddPropertyArray.isEqual(other.AddPropertyArray))
        return false;
      if (!Properties.isEqual(other.Properties))
        return false;
      if (!ArbitraryArray.isEqual(other.ArbitraryArray))
        return false;
      return true;
    }
    ...
    // 2021-04 : Following is temporary to compare with old way of storing
    // properties. Let's keep it few months until I am sure the refactoring
    // isomorphic
  private:
    DEV_PROPERTY *ArbProperties = 0; // the old version.
  public:
    void FillDevicePropertiesOld(SETTINGS_DATA &gSettings,
                                 const TagDict *DevicesDict);

    XBool compareDevProperty(const XString8 &label, const DEV_PROPERTY &oldProp,
                             const DEV_PROPERTY &newProp) const {
      ...
      if (newProp.ValueLen != oldProp.ValueLen) {
        printf("%s.Value.ValueLen %lld != %lld\n", label.c_str(),
               oldProp.ValueLen, newProp.ValueLen);
        return false;
      } else {
      ...
      while (oldChild && newChild) {
        compareDevProperty(S8Printf("%s.child[%zu]", label.c_str(), jdx),
                           *oldChild, *newChild);
    ...
    XBool compareOldAndCompatibleArb() {
      ...
      //            arb->Key); arb = arb->Next;
      //          }
      //        }
      const XObjArray<DEV_PROPERTY> compatibleArbProperties =
          getCompatibleArbProperty();
      //      size_t oldArbIdx = 0;
      for (size_t idx = 0; idx < compatibleArbProperties.size(); ++idx) {
        ...
        if (!compareDevProperty(S8Printf("ArbProperties[%zu]", idx),
                                *ArbProperties, compatibleArbProperties[idx])) {
          return false;
        }
        //         ++oldArbIdx;
    ...
    XObjArray<DEV_PROPERTY> getCompatibleArbProperty() const {
      ...
      for (size_t idx = ArbitraryArray.size(); idx-- > 0;) {
        ...
        for (size_t jdx = newArb.CustomPropertyArray.size(); jdx-- > 0;) {
          const SimplePropertyClass &newArbProp =
              newArb.CustomPropertyArray[jdx];
          DEV_PROPERTY *newProp = new DEV_PROPERTY;
          newProp->Device = newArb.Device;
          newProp->Key = const_cast<char *>(
              newArbProp.Key
                  .c_str()); // const_cast !!! So ugly. It is just because it's
                             // temporary. If ArbProperties is modified after
                             // this, a lot a memory problem will happen. I
                             // could have done some strdup, but that way I
                             // don't use memory and don't have to free it.
          ...
          //            newProp->Key.c_str(), newProp->Label.c_str(),
          //            arb.size()-1);
        }
      }
      // Non arb : device = 0
      for (size_t idx = Properties.PropertyArray.size(); idx-- > 0;) {
        const PropertiesClass::PropertyClass &Prop =
            Properties.PropertyArray[idx];
        DEV_PROPERTY *newProp = new DEV_PROPERTY;
        newProp->Device = 0;
        newProp->Key = 0;
        if (Prop.Enabled)
          newProp->Label =
              XString8(Prop.DevicePathAsString).forgetDataWithoutFreeing();
        else
          newProp->Label = S8Printf("!%ls", Prop.DevicePathAsString.wc_str())
                               .forgetDataWithoutFreeing();
        newProp->Child = NULL;
        for (size_t jdx = Properties.PropertyArray[idx].propertiesArray.size();
             jdx-- > 0;) {
  ...
  public:
    ...
    public:
      ...
      XBool isEqual(const OcKernelQuirksClass &other) const {
        if (!(AppleXcpmExtraMsrs == other.AppleXcpmExtraMsrs))
          return false;
        if (!(AppleXcpmForceBoost == other.AppleXcpmForceBoost))
          return false;
        if (!(DisableIoMapper == other.DisableIoMapper))
          return false;
        if (!(DisableIoMapperMapping == other.DisableIoMapperMapping))
          return false;
        if (!(DisableLinkeditJettison == other.DisableLinkeditJettison))
          return false;
        if (!(DummyPowerManagement == other.DummyPowerManagement))
          return false;
        if (!(ExtendBTFeatureFlags == other.ExtendBTFeatureFlags))
          return false;
        if (!(ExternalDiskIcons == other.ExternalDiskIcons))
          return false;
        if (!(IncreasePciBarSize == other.IncreasePciBarSize))
          return false;
        if (!(ForceAquantiaEthernet == other.ForceAquantiaEthernet))
          return false;
        if (!(PowerTimeoutKernelPanic == other.PowerTimeoutKernelPanic))
          return false;
        if (!(ThirdPartyDrives == other.ThirdPartyDrives))
          return false;
        if (!(XhciPortLimit == other.XhciPortLimit))
          return false;
      ...
      void takeValueFrom(
          const ConfigPlistClass::Quirks_Class::OcKernelQuirks_Class &other) {
        ...
        DisableIoMapperMapping = other.dgetDisableIoMapperMapping();
    ...
    public:
      ...
      int8_t ResizeAppleGpuBars =
          0; // 0 is NOT the default value if not set in config.plist. Default
             // value if not set is returned by dgetResizeAppleGpuBars()
      int8_t ResizeGpuBars = -1;
      ...
      XBool isEqual(const OcBooterQuirksClass &other) const {
        if (!(AvoidRuntimeDefrag == other.AvoidRuntimeDefrag))
          return false;
        if (!(DevirtualiseMmio == other.DevirtualiseMmio))
          return false;
        if (!(DisableSingleUser == other.DisableSingleUser))
          return false;
        if (!(DisableVariableWrite == other.DisableVariableWrite))
          return false;
        if (!(DiscardHibernateMap == other.DiscardHibernateMap))
          return false;
        if (!(EnableSafeModeSlide == other.EnableSafeModeSlide))
          return false;
        if (!(EnableWriteUnprotector == other.EnableWriteUnprotector))
          return false;
        if (!(ForceExitBootServices == other.ForceExitBootServices))
          return false;
        if (!(ProtectSecureBoot == other.ProtectSecureBoot))
          return false;
        if (!(ProtectUefiServices == other.ProtectUefiServices))
          return false;
        if (!(ProtectUefiServices == other.ProtectUefiServices))
          return false;
        if (!(ProvideCustomSlide == other.ProvideCustomSlide))
          return false;
        if (!(ProvideMaxSlide == other.ProvideMaxSlide))
          return false;
        if (!(RebuildAppleMemoryMap == other.RebuildAppleMemoryMap))
          return false;
        if (!(ResizeAppleGpuBars == other.ResizeAppleGpuBars))
          return false;
        if (!(ResizeGpuBars == other.ResizeGpuBars))
          return false;
        if (!(SetupVirtualMap == other.SetupVirtualMap))
          return false;
        if (!(SignalAppleOS == other.SignalAppleOS))
          return false;
        if (!(SyncRuntimePermissions == other.SyncRuntimePermissions))
          return false;
        if (!(ForceOcWriteFlash == other.ForceOcWriteFlash))
          return false;
        if (!(TscSyncTimeout == other.TscSyncTimeout))
          return false;
        return true;
      }
      void takeValueFrom(
          const ConfigPlistClass::Quirks_Class::OcBooterQuirks_Class &other) {
        ...
        ResizeGpuBars = other.dgetResizeGpuBars();
    ...
    XObjArrayWithTakeValueFromXmlArray<
        ...
        mmioWhiteListArray = XObjArrayWithTakeValueFromXmlArray<
            MMIOWhiteList,
            ConfigPlistClass::Quirks_Class::Quirks_MmioWhitelist_Class>();
    UINT32 QuirksMask = 0;
    ...
    XBool isEqual(const QuirksClass &other) const {
      if (!(FuzzyMatch == other.FuzzyMatch))
        return false;
      if (!(OcKernelCache == other.OcKernelCache))
        return false;
      if (!(OcKernelQuirks.isEqual(other.OcKernelQuirks)))
        return false;
      if (!(OcBooterQuirks.isEqual(other.OcBooterQuirks)))
        return false;
      if (!mmioWhiteListArray.isEqual(other.mmioWhiteListArray))
        return false;
      if (!(QuirksMask == other.QuirksMask))
        return false;
      return true;
    }
  ...
  public:
    ...
    public:
      ...
      XBool isEqual(const RT_VARIABLES &other) const {
        ...
        if (!(Name == other.Name))
          return false;
        if (memcmp(&Guid, &other.Guid, sizeof(Guid)) != 0)
          return false;
        return true;
      }
      void takeValueFrom(
          const ConfigPlistClass::RtVariables_Class::Devices_RtVariables_Block
              &other) {
        Disabled = other.dgetDisabled();
        Comment = other.dgetComment();
        Name = other.dgetName();
    ...
    XObjArrayWithTakeValueFromXmlArray<
        RT_VARIABLES,
        ConfigPlistClass::RtVariables_Class::Devices_RtVariables_Block>
        BlockRtVariableArray = XObjArrayWithTakeValueFromXmlArray<
            RT_VARIABLES,
            ConfigPlistClass::RtVariables_Class::Devices_RtVariables_Block>();

    XBool GetLegacyLanAddress() const {
      return RtROMAsString.isEqualIC("UseMacAddr0") ||
             RtROMAsString.isEqualIC("UseMacAddr1");
    }
    ...
    XBool isEqual(const RtVariablesClass &other) const {
      if (!(RtROMAsString == other.RtROMAsString))
        return false;
      if (!(RtROMAsData == other.RtROMAsData))
        return false;
      if (!(RtMLBSetting == other.RtMLBSetting))
        return false;
      if (!(CsrActiveConfig == other.CsrActiveConfig))
        return false;
      if (!(BooterConfig == other.BooterConfig))
        return false;
      if (!(BooterCfgStr == other.BooterCfgStr))
        return false;
      if (!(HWTarget == other.HWTarget))
        return false;
      if (!BlockRtVariableArray.isEqual(other.BlockRtVariableArray))
        return false;
      return true;
    }
  ...
  public:
    ...
    public:
      ...
      XBool isEqual(const SlotDeviceClass &other) const {
        if (!(SmbiosIndex == other.SmbiosIndex))
          return false;
        if (!(SlotID == other.SlotID))
          return false;
        if (!(SlotType == other.SlotType))
          return false;
        if (!(SlotName == other.SlotName))
          return false;
        return true;
      }
      void takeValueFrom(
          const SmbiosPlistClass::SmbiosDictClass::SlotDeviceDictClass &other) {
    ...
    class SlotDeviceArrayClass
        : public XObjArrayWithTakeValueFromXmlArray<
              SlotDeviceClass,
              SmbiosPlistClass::SmbiosDictClass::SlotDeviceDictClass> {
    public:
      XBool doesSlotForIndexExist(uint8_t idx2Look4) const {
        for (size_t idx = 0; idx < size(); ++idx) {
          if (ElementAt(idx).SmbiosIndex == idx2Look4)
            return true;
        }
        return false;
      }
    ...
    public:
      ...
      XBool takeValueFrom(const SmbiosPlistClass::SmbiosDictClass::
                              MemoryDictClass::ModuleDictClass &other) {
    ...
    public:
      ...
      XObjArrayWithTakeValueFromXmlArray<
          RamSlotInfo,
          SmbiosPlistClass::SmbiosDictClass::MemoryDictClass::ModuleDictClass>
          User = XObjArrayWithTakeValueFromXmlArray<
              RamSlotInfo, SmbiosPlistClass::SmbiosDictClass::MemoryDictClass::
                               ModuleDictClass>();
      ...
      XBool isEqual(const RamSlotInfoArrayClass &other) const {
        if (!(SlotCount == other.SlotCount))
          return false;
        if (!(UserChannels == other.UserChannels))
          return false;
        if (!(User.isEqual(other.User)))
          return false;
        return true;
      }
      void takeValueFrom(
          const SmbiosPlistClass::SmbiosDictClass::MemoryDictClass &other) {
    ...
    XBool isEqual(const SmbiosClass &other) const {
      // SMBIOS TYPE0
      if (!(BiosVendor == other.BiosVendor))
        return false;
      if (!(BiosVersion == other.BiosVersion))
        return false;
      if (!(EfiVersion == other.EfiVersion))
        return false;
      if (!(BiosReleaseDate == other.BiosReleaseDate))
        return false;
      // SMBIOS TYPE1
      if (!(ManufactureName == other.ManufactureName))
        return false;
      if (!(ProductName == other.ProductName))
        return false;
      if (!(SystemVersion == other.SystemVersion))
        return false;
      if (!(SerialNr == other.SerialNr))
        return false;
      if (!(SmUUID == other.SmUUID))
        return false;
      if (!(FamilyName == other.FamilyName))
        return false;
      // SMBIOS TYPE2
      if (!(BoardManufactureName == other.BoardManufactureName))
        return false;
      if (!(BoardSerialNumber == other.BoardSerialNumber))
        return false;
      if (!(BoardNumber == other.BoardNumber))
        return false;
      if (!(LocationInChassis == other.LocationInChassis))
        return false;
      if (!(BoardVersion == other.BoardVersion))
        return false;
      ...
      if (!(Mobile == other.Mobile))
        return false;
      if (!(ChassisType == other.ChassisType))
        return false;
      if (!(ChassisManufacturer == other.ChassisManufacturer))
        return false;
      if (!(ChassisAssetTag == other.ChassisAssetTag))
        return false;
      // SMBIOS TYPE4
      if (!(SFakeCPU == other.SFakeCPU))
        return false;
      // SMBIOS TYPE17
      if (!(SmbiosVersion == other.SmbiosVersion))
        return false;
      if (!(Attribute == other.Attribute))
        return false;
      // SMBIOS TYPE132
      if (!(TrustSMBIOS == other.TrustSMBIOS))
        return false;
      if (!(InjectMemoryTables == other.InjectMemoryTables))
        return false;
      // SMBIOS TYPE133
      if (!(gPlatformFeature == other.gPlatformFeature))
        return false;
      // PatchTableType11
      if (!(NoRomInfo == other.NoRomInfo))
        return false;

      if (!(FirmwareFeatures == other.FirmwareFeatures))
        return false;
      if (!(FirmwareFeaturesMask == other.FirmwareFeaturesMask))
        return false;
      if (!(ExtendedFirmwareFeatures == other.ExtendedFirmwareFeatures))
        return false;
      if (!(ExtendedFirmwareFeaturesMask == other.ExtendedFirmwareFeaturesMask))
        return false;
      if (!RamSlotInfoArray.isEqual(other.RamSlotInfoArray))
        return false;
      if (!SlotDevices.isEqual(other.SlotDevices))
        return false;
  ...
  public:
    ...
    XBool isEqual(const BootGraphicsClass &other) const {
      if (!(DefaultBackgroundColor == other.DefaultBackgroundColor))
        return false;
      if (!(UIScale == other.UIScale))
        return false;
      if (!(EFILoginHiDPI == other.EFILoginHiDPI))
        return false;
      if (_flagstate != other._flagstate)
        return false;
      return true;
    }
  ...
  SETTINGS_DATA() {}
  //  SETTINGS_DATA(const SETTINGS_DATA& other) = delete; // Can be defined if
  //  needed const SETTINGS_DATA& operator = ( const SETTINGS_DATA & ) = delete;
  ...
  XBool isEqual(const SETTINGS_DATA &other) const {
    if (!Boot.isEqual(other.Boot))
      return false;
    if (!ACPI.isEqual(other.ACPI))
      return false;
    if (!GUI.isEqual(other.GUI))
      return false;
    if (!CPU.isEqual(other.CPU))
      return false;
    if (!SystemParameters.isEqual(other.SystemParameters))
      return false;
    if (!KernelAndKextPatches.isEqual(other.KernelAndKextPatches))
      return false;
    if (!Graphics.isEqual(other.Graphics))
      return false;
    if (!(DisabledDriverArray == other.DisabledDriverArray))
      return false;
    if (!Quirks.isEqual(other.Quirks))
      return false;
    if (!RtVariables.isEqual(other.RtVariables))
      return false;
    if (!Devices.isEqual(other.Devices))
      return false;
    if (!Smbios.isEqual(other.Smbios))
      return false;
    if (!BootGraphics.isEqual(other.BootGraphics))
      return false;
    return true;
  }
  ...
  XBool ShouldInjectSystemID() {
    if (SystemParameters.CustomUuid
            .notNull() /*&&  SystemParameters.CustomUuid.notNull()*/) {
      if (SystemParameters._InjectSystemID == 2)
        return false;
      else
        return SystemParameters._InjectSystemID != 0;
    }
    if (Smbios.SmUUID.isNull() /* || Smbios.SmUUID.isNull() */)
      return false;
    if (SystemParameters._InjectSystemID == 2)
      return true;
    return SystemParameters._InjectSystemID != 0;
  ...
  XBool getEnableC6() const {
    if (CPU._EnableC6.isDefined())
      return CPU._EnableC6.value();
    return ACPI.SSDT._EnableC6;
  }
  XBool getEnableC4() const {
    if (CPU._EnableC4.isDefined())
      return CPU._EnableC4.value();
    return ACPI.SSDT._EnableC4;
  }
  XBool getEnableC2() const {
    if (CPU._EnableC2.isDefined())
      return CPU._EnableC2.value();
...
public:
  ...
  ACPI_PATCHED_AML() {};
  ACPI_PATCHED_AML(const ACPI_PATCHED_AML &other) =
      delete; // Can be defined if needed
  const ACPI_PATCHED_AML &
  operator=(const ACPI_PATCHED_AML &) = delete; // Can be defined if needed
  ~ACPI_PATCHED_AML() {}
};
...
public:
  ...
  SIDELOAD_KEXT(const SIDELOAD_KEXT &other) =
      delete; // Can be defined if needed
  const SIDELOAD_KEXT &
  operator=(const SIDELOAD_KEXT &) = delete; // Can be defined if needed
  ~SIDELOAD_KEXT() {}
};
...
class REFIT_CONFIG {
public:
  UINTN DisableFlags = 0; // to disable some volume types (optical, firewire
                          // etc)
  XBool Quiet = true;
  ...
  UINT8 CustomLogoType =
      0; // this will be initialized with gSettings.Boot.CustomBoot and set back
         // to CUSTOM_BOOT_DISABLED if CustomLogo could not be loaded or decoded
         // (see afterGetUserSettings)
  XImage *CustomLogo = 0;

  XBool DropSSDT =
      false; // init with gSettings.Boot.DropSSDTSetting. Put back to false if
             // one table is dropped (see afterGetUserSettings)
  ...
  XObjArray<CUSTOM_LOADER_ENTRY> CustomEntries =
      XObjArray<CUSTOM_LOADER_ENTRY>();
  XObjArray<CUSTOM_LEGACY_ENTRY> CustomLegacyEntries =
      XObjArray<CUSTOM_LEGACY_ENTRY>();
  XObjArray<CUSTOM_TOOL_ENTRY> CustomToolsEntries =
      XObjArray<CUSTOM_TOOL_ENTRY>();
  ...
  XBool KernelPatchesAllowed =
      true; // From GUI: Only for user patches, not internal Clover
  ...
  const REFIT_CONFIG &
  operator=(const REFIT_CONFIG &) = delete; // Can be defined if needed
  ~REFIT_CONFIG() {}
...
EFI_STATUS
InjectKextsFromDir(EFI_STATUS Status, CHAR16 *SrcDir);
