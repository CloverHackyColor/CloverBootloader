...
declare NOEXTRAS=""
declare -rx duplicateSuffix='.another'
...
addChoice () {
    ...
    # the title/description can now have a suffix indicating that is a duplicate driver's choice
    local ltitle="${choiceId%${duplicateSuffix}*}"
    local ldescription="${choiceId%${duplicateSuffix}*}"

    if grep -q "\"${ltitle}_title\"" "${PKGROOT}/Resources/templates/Localizable.strings"; then
      ltitle="${ltitle}_title"
    else
      ltitle="${ltitle%.UEFI*}"
    fi

    if grep -q "\"${ldescription}_description\"" "${PKGROOT}/Resources/templates/Localizable.strings"; then
      ldescription="${ldescription}_description"
    else
      ldescription="${ldescription%.UEFI*}"
    fi
...
# build mandatory drivers-ia32 packages
    if [[ ${NOEXTRAS} != *"CloverEFI"* ]]; then
        ...
        do
            ...
            # Check that the choice doesn't already exists.. allow duplicate drivers (but with different id)
            tempDriverChoice="$driverChoice"
            dplc=0
            idx=$(getChoiceIndex "$tempDriverChoice")
            found=$?
            while [[ $found -ne 0 ]] ; do
              ((dplc+=1))
              tempDriverChoice="${driverChoice}${duplicateSuffix}${dplc}"
              idx=$(getChoiceIndex "$tempDriverChoice")
              found=$?
            done

            ditto --noextattr --noqtn --arch i386 "${drivers[$i]}" "${PKG_BUILD_DIR}/${tempDriverChoice}/Root/"
            find "${PKG_BUILD_DIR}/${tempDriverChoice}" -name '.DS_Store' -exec rm -R -f {} \; 2>/dev/null
            fixperms "${PKG_BUILD_DIR}/${tempDriverChoice}/Root/"

            packageRefId=$(getPackageRefId "${packagesidentity}" "${tempDriverChoice}")
            # Add postinstall script for VBoxHfs driver to remove it if HFSPlus driver exists
            [[ "$driver" == VBoxHfs* ]] && \
            addTemplateScripts --pkg-rootdir="${PKG_BUILD_DIR}/${tempDriverChoice}"  \
                                --subst="DRIVER_NAME=$driver"                     \
                                --subst="DRIVER_DIR=$(basename $driverDestDir)"   \
                                "VBoxHfs"
            buildpackage "$packageRefId" "${tempDriverChoice}" "${PKG_BUILD_DIR}/${tempDriverChoice}" "${driverDestDir}"
            addChoice --start-visible="true" \
                      ...
                      --pkg-refs="$packageRefId"  "${tempDriverChoice}"
            rm -R -f "${PKG_BUILD_DIR}/${tempDriverChoice}"
        done
        ...
        for (( i = 0 ; i < ${#drivers[@]} ; i++ )); do
            ...
            addChoice --group="Drivers32" --title="$driverName"                                               \
                    --enabled="!choices['UEFI.only'].selected"                                              \
                    --start-selected="choicePreviouslySelected('$packageRefId')"                            \
                    --selected="!choices['UEFI.only'].selected &amp;&amp; choices['$tempDriverChoice'].selected"  \
    ...
    do
        ...
        [[ "$driver" == VBoxHfs* ]] && \
         addTemplateScripts --pkg-rootdir="${PKG_BUILD_DIR}/${tempDriverChoice}"   \
                            ...
                            --subst="DRIVER_DIR=$(basename $driverDestDir)"  \
                            "VBoxHfs"
        buildpackage "$packageRefId" "${tempDriverChoice}" "${PKG_BUILD_DIR}/${tempDriverChoice}" "${tempDriverChoice}"
        addChoice --start-visible="true" --start-selected="true" --pkg-refs="$packageRefId" "${tempDriverChoice}"
        rm -R -f "${PKG_BUILD_DIR}/${tempDriverChoice}"
    done
...
if [[ -d "${SRCROOT}/CloverV2/drivers-Off/drivers64" && ${NOEXTRAS} != *"CloverEFI"* ]]; then
    ...
    for (( i = 0 ; i < ${#drivers[@]} ; i++ )); do
        ...
        while [[ $found -ne 0 ]] ; do
          ((dplc+=1))
          tempDriverChoice="${driverName}${duplicateSuffix}${dplc}"
          idx=$(getChoiceIndex "$tempDriverChoice")
          found=$?
        done
        ...
        addTemplateScripts --pkg-rootdir="${PKG_BUILD_DIR}/${tempDriverChoice}" \
                           --subst="INSTALLER_CHOICE=$packageRefId" MarkChoice
        buildpackage "$packageRefId" "${tempDriverChoice}" "${PKG_BUILD_DIR}/${tempDriverChoice}" "${driverDestDir}"
        addChoice --group="Drivers64" --title="$tempDriverChoice"                                               \
                  --enabled="!choices['UEFI.only'].selected"                                              \
                  --start-selected="!choices['UEFI.only'].selected &amp;&amp; choicePreviouslySelected('$packageRefId')"                            \
                  --selected="choices['$tempDriverChoice'].selected"  \
                  --pkg-refs="$packageRefId"                                                              \
                  "${tempDriverChoice}"
        rm -R -f "${PKG_BUILD_DIR}/${tempDriverChoice}"
    done
    ...
    do
        ...
        [[ "$driver" == VBoxHfs* ]] && \
         addTemplateScripts --pkg-rootdir="${PKG_BUILD_DIR}/${tempDriverChoice}"  \
                            ...
                            "VBoxHfs"
        buildpackage "$packageRefId" "${tempDriverChoice}" "${PKG_BUILD_DIR}/${tempDriverChoice}" "${driverDestDir}"
        addChoice --group="Drivers64UEFI" --start-visible="true" --start-selected="true" --pkg-refs="$packageRefId"  "${tempDriverChoice}"
        rm -R -f "${PKG_BUILD_DIR}/${tempDriverChoice}"
    done
fi
...
if [[ -d "${SRCROOT}/CloverV2/drivers-Off/drivers64UEFI" ]]; then
    ...
    do
        ...
        addTemplateScripts --pkg-rootdir="${PKG_BUILD_DIR}/${tempDriverChoice}" \
                           --subst="INSTALLER_CHOICE=$packageRefId" MarkChoice
        buildpackage "$packageRefId" "${tempDriverChoice}" "${PKG_BUILD_DIR}/${tempDriverChoice}" "${driverDestDir}"
        addChoice --group="Drivers64UEFI"  --title="$tempDriverChoice"                \
                  --start-selected="choicePreviouslySelected('$packageRefId')"  \
                  --pkg-refs="$packageRefId"  "${tempDriverChoice}"
        rm -R -f "${PKG_BUILD_DIR}/${tempDriverChoice}"
    done
