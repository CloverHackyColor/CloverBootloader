...
GetBestLanguage (
  IN CONST CHAR8  *SupportedLanguages,
  IN BOOLEAN      Iso639Language,   //false
  IN CONST CHAR8  *Lang,
  ...
  )
{
  ...
  ASSERT (SupportedLanguages != NULL);  //test SupportedLanguages="en", Lang="en"

  VA_START (Args, Lang);
  while ((Language = VA_ARG (Args, CHAR8 *)) != NULL) { //Language = "en"
    //
    // Default to ISO 639-2 mode
    //
    CompareLength  = 3;
    LanguageLength = MIN (3, AsciiStrLen (Language)); //==2

    //
    // If in RFC 4646 mode, then determine the length of the first RFC 4646 language code in Language
    //
    if (!Iso639Language) {
      for (LanguageLength = 0; Language[LanguageLength] != 0 && Language[LanguageLength] != ';'; LanguageLength++); //LanguageLength = 2
    }
    ...
    while (LanguageLength > 0) {
      ...
      //
      for (Supported = SupportedLanguages; *Supported != '\0'; Supported += CompareLength) {  //1. *Supported == 'e'
        //
        ...
        if (!Iso639Language) {
          ...
          //
          for (; *Supported != '\0' && *Supported == ';'; Supported++); //*Supported == 'e'
          //
          // Determine the length of the next language code in Supported
          //
          for (CompareLength = 0; Supported[CompareLength] != 0 && Supported[CompareLength] != ';'; CompareLength++); //Supported[0] == 'e' , CompareLength == 2
          //
          // If Language is longer than the Supported, then skip to the next language
          //
          if (LanguageLength > CompareLength) { //2 > 2 false
            continue;
          }
        }
        //
